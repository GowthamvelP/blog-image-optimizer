...
| **Storage** | Local filesystem — ephemeral on Render (images lost on redeploy) |
| **No persistence** | No database; no upload history or management UI |
| **No auth** | Anyone with the URL can upload |
| **No CDN** | Images served directly from the app; no external storage (S3/R2) |
| **No deduplication** | Uploading the same image twice creates new output files |
| **SVG handling** | SVG files are rasterized during resize (not kept as vector) |
| **Cold starts** | Render free tier may spin down after inactivity, causing slow first load |
